...
var (
	...
	splitwiseClient     splitwise.Client = nil
)
...
func init() {
	...
	splitwiseClient = splitwise.NewClient(auth)
}
...
func main() {
	...
	r.GET("/api/splitwise/groups", getSplitwiseGroups)
	r.POST("/plaid/webhook", plaidWebhook)
	r.GET("/something", trialSomething)
	err := r.Run(":" + APP_PORT)
	if err != nil {
...
func linkTokenCreate(
	...
	request.SetWebhook("https://engsgoe73xsa9.x.pipedream.net")
...
func getSplitwiseGroups(c *gin.Context) {
	ctx := context.Background()
...
func trialSomething(c *gin.Context){
	ctx := context.Background()
